...
class CreateTestCustomers extends Command
{
     ...
     * @var string
     */
    protected $signature = 'customers:create-test {--count=10 : Number of customers to create}';
     ...
     * @var string
     */
    protected $description = 'Create test customers with all related data (emergency contacts, insurance, accommodation)';
    ...
    public function handle()
    {
        ...
        $customersData = array_slice($customersData, 0, $count);
        ...
        try {
            ...
            if (!$diveCenter) {
                ...
                $this->info("Created dive center: {$diveCenter->name} (ID: {$diveCenter->id})");
            } else {
                $this->info("Using existing dive center: {$diveCenter->name} (ID: {$diveCenter->id})");
            }
            ...
            foreach ($customersData as $data) {
                ...
                if ($insuranceData) {
                    ...
                    CustomerInsurance::create($insuranceData);
                }

                // Create accommodation
                if ($accommodationData) {
                    $accommodationData['customer_id'] = $customer->id;
                    CustomerAccommodation::create($accommodationData);
                }
            ...
            $this->newLine(2);
            $this->info("✓ Successfully created {$createdCount} customers with all related data!");
            $this->newLine();
            ...
            DB::rollBack();
            $this->error("Failed to create customers");
